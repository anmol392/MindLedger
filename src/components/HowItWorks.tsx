    ...
    <section className="py-24 bg-zinc-950/50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center text-center">
          <h2 className="text-3xl font-bold text-white uppercase tracking-tighter sm:text-4xl">Platform Mechanics</h2>
        ...
        <div className="mt-20 grid grid-cols-1 gap-12 lg:grid-cols-4 lg:gap-8">
          {steps.map((step, index) => (
            ...
            >
              ...
              <h3 className="mb-3 text-lg font-bold text-white uppercase tracking-tight">{step.title}</h3>
              <p className="text-center text-sm leading-relaxed text-zinc-400 px-4">{step.description}</p>
              
              <div className="mt-6 flex size-8 items-center justify-center rounded-full bg-white/5 border border-white/10 text-xs font-mono font-bold text-zinc-600 group-hover:border-primary group-hover:text-primary transition-colors">
                0{index + 1}
              </div>
            </motion.div>
          ))}
