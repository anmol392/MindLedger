...
export function useProctoringSession(problemId: string) {
  ...
  useEffect(() => {
    ...
    // 2. Microphone Monitoring (Voice/Noise Detection)
    ...
    const startMicMonitoring = (mStream: MediaStream) => {
      ...
      scriptProcessor.onaudioprocess = () => {
        ...
        // Threshold for "voice" detection (calibrated for standard mic sensitivity)
        if (average > 65) { 
          setWarningCount((prev) => {
             const next = prev + 1;
             console.warn(`[PROCTOR] Voice/Noise detected. Warning ${next}/3`);
             return next;
          });
          // Disable processing temporarily to debounce warnings
    ...
    // 2.5 Motion Detection (Webcam Pixel Analysis)
    const startMotionDetection = (mStream: MediaStream) => {
      ...
      motionInterval = setInterval(() => {
        ...
        if (prevFrame) {
          ...
          if (moveRatio > 0.40) { // Large movement threshold
            console.warn("[PROCTOR] Excessive motion detected:", moveRatio);
            setWarningCount((prev) => {
              const next = prev + 1;
              alert("⚠️ EXCESSIVE MOTION DETECTED: Please remain still and focused. This action has been logged.");
              return next;
            });
            // Debounce
